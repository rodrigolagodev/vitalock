-- ============================================================
-- Migration: equipment_replacement uses stock product, not equipment instance
-- ============================================================
-- Reality check: `intended_replacement_equipment_id` (added in 100/101) points
-- to an operations.equipment row, but operations only stores REAL installed
-- devices with unique serials. A "replacement from warehouse" is not an
-- installed device — it's a stock item (public.products with
-- category='equipment') that becomes a device only when the installer types
-- the serial in the field (resolve_equipment_replacement creates the row).
--
-- Fix: reuse the existing `product_id` column on technical_order_items for
-- equipment_replacement items (same pattern as item_type='equipment'). The
-- confirm-time stock reservation logic already fires whenever product_id is
-- not null, so no new column is needed. `intended_replacement_equipment_id`
-- stays in the schema (nullable, unused) to avoid destructive migration.
--
-- Additional validation: for item_type in ('equipment','equipment_replacement')
-- the referenced product must be category='equipment'; for item_type='key'
-- it must be category='rfid_key'. Enforced in the create/confirm RPCs.
--
-- resolve_equipment_replacement: p_new_model becomes optional. When the ticket
-- comes from a technical_order_item with product_id set, the RPC auto-fills
-- p_new_model from products.name.
...
-- -------------------------------------------------------
-- resolve_equipment_replacement — auto-fill model from product name
-- -------------------------------------------------------
-- When the ticket comes from a technical_order_item with product_id set,
-- p_new_model becomes optional and defaults to products.name. The installer
-- (or admin from the tareas panel) still supplies the serial; the model is
-- implied by the product picked at order time.
