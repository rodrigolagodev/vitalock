...
/**
 * Per-key pickup registration for order-produced keys (task 2.4). Submits via
 * useMutateKey.recordPickup → rpc record_order_key_pickup, which validates the
 * DNI against the order's authorized particulares (buyer or pickup person) and
 * auto-completes the order when all key items have been picked up.
 */
export function PickupKeyDialog({
  ...
  // Prefill on open: the dialog stays mounted, so defaultValues only apply at
  // mount — reset whenever the dialog opens (or the pickup person changes).
  useEffect(() => {
    if (!open) return;
