-- ============================================================
-- Migration: resolve_equipment_update v2 — return JSONB with skipped keys
-- ============================================================
-- Changes the return type from uuid to jsonb so callers can
-- surface which keys were skipped during resolution.
--
-- Return shape: {"ticket_id": <uuid>, "skipped_key_ids": [<uuid>, ...]}
--
-- PostgreSQL does not allow CREATE OR REPLACE to change a function's
-- return type, so we DROP and recreate.
-- ============================================================

drop function if exists public.resolve_equipment_update(uuid, uuid);
