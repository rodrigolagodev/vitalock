...
- **WHEN** a developer runs `git commit` with a staged `.tsx` file containing prettier or eslint errors
- **THEN** the Husky `pre-commit` hook runs `lint-staged` which invokes `prettier --write` and `eslint --fix` on the staged file
- **AND** the commit is either automatically fixed and proceeds, or fails and blocks the commit with the raw eslint error output
